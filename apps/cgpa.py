...
pickle_in = open('Student_Classifier_CgpaPrediction.pkl', 'rb')
...
def clear_text():
    ...
    st.session_state["entrancetaken"] = ""
    st.session_state["rankofentrance"] = ""
    st.session_state["seattype"] = ""
...
def app():
    ...
    html_temp = '<p style= "color:Black; font-size: 20px;">CGPA Prediction of Students</p>'
    st.markdown(html_temp, unsafe_allow_html = True)
    tenthCgpa = st.text_input("10th CGPA", key="tenthCgpa")
    percent_12 = st.text_input("12th %", key="percent_12")
    entrancetaken = st.text_input("Entrance Taken", key="entrancetaken")
    rankofentrance = st.text_input("Rank of Entrance Test", key="rankofentrance")
    seattype = st.text_input("Seat Type",key="seattype")
